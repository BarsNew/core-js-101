...
function doRectanglesOverlap(rect1, rect2) {
  ...
  const isOverlapX = left1 < right2 && right1 > left2;
  const isOverlapY = top1 < bottom2 && bottom1 > top2;
...
function isInsideCircle(circle, point) {
  const r = ((circle.center.x - point.x) ** 2 + (circle.center.y - point.y) ** 2);

  return r < circle.radius ** 2;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  const reightBracket = (isEndIncluded) ? ']' : ')';
  let newA = a;
  let newB = b;
  if (b < a) [newA, newB] = [newB, newA];

  return `${leftBracket}${newA}, ${newB}${reightBracket}`;
}
...
function isCreditCardNumber(ccn) {
  ...
  digits = digits.replace(/\D/g, '').split('').map(Number);
...
function getDigitalRoot(num) {
  let count = Array.from(String(num));
  count = count.reduce((acc, item) => acc + +item, 0);
  if (count > 9) return getDigitalRoot(count);
  return count;
}
...
function isBracketsBalanced(str) {
  const checkStr = Array.from(String(str));
  const stack = [];

  if (checkStr.length % 2 !== 0) return false;
  ...
  for (let i = 0; i < checkStr.length; i += 1) {
    ...
    if (isOpening(currentChar)) {
      ...
      if (getM(currentChar) === openBracket) {
        stack.pop();
      } else {
        return false;
      }
...
function getCommonDirectoryPath(pathes) {
  const diffPathes = pathes.map((item) => item.split('/'));
  const minPath = Math.min(...diffPathes.map((parts) => parts.length));
  const dirs = [];
  let isSlash = false;

  for (let i = 0; i < minPath; i += 1) {
    const dir = diffPathes[0][i];

    const isDir = diffPathes.every((parts) => parts[i] === dir);

    isSlash = pathes.every((parts) => parts[0] === '/');
...
function getMatrixProduct(m1, m2) {
  ...
  return res.map((row, i) => row.map((_, j) => m1[i]
...
function evaluateTicTacToePosition(position) {
  ...
  if (position.every((row) => row.every((cell) => cell))) {
    return undefined;
  }
