...
function isLeapYear(date) {
  const nextMonth = new Date(date.getFullYear(), 1, 29);
...
function timeSpanToString(startDate, endDate) {
  ...
  return `${formatTime(hours)}:${formatTime(minutes)}:${formatTime(seconds)}.${formatMiliTime(milliseconds)}`;
}
...
function angleBetweenClockHands(date) {
  ...
  const hours = dat.getUTCHours() % 12;
